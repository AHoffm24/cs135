...
int main()
{
  ...
  string rentals[40]; //string array to hold License# of each rented vehicle
  inventory info[40]; //array of max # of vehichles to be read
...
double ProcessRentals(ofstream& out,inventory info[], string rentals[], int countI, int& countR)
{
  ...
  for(int i = 0; i < countR; i++)
  {
    for(int j = 0; j < countI;j++)
    {
      ...
      {
        info[j].status = false; //telling the structure the car is rented

        out << "License# " << rentals[i] << " "; //which car is being rented
        ...
        j = countI; //stops j loop, car found and status changed get next rental
      }
    }
